 ...
 */
function get({ url, config, contents, query, useCookie = false }) {
    return new Promise((resolve) => {
        ...
        // 只有在需要时才添加cookie
        if (useCookie) {
            headers['cookie'] = GlobalVar.get('cookie');
        }
 ...
 * @returns {Promise<string>}
 */
function post({ url, config, contents, query, useCookie = true }) {
    return new Promise((resolve) => {
        const headers = {
            'accept': 'application/json, text/plain, */*',
...
const bili_client = {
    ...
    getOneArticleByCv(cv) {
        return get({
            ...
            useCookie: false  // 获取专栏内容不需要cookie
        });
    },
    /**
     * 获取粉丝数的所有有效方式
