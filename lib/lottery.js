...
function start(num) {
    return new Promise(resolve => {
        ...
        event_bus.on('Turn_on_the_Monitor', async () => {
            ...
            // 使用程序开始运行时的小时，而不是当前小时，确保所有账号使用相同的逻辑
            const runHour = process.env.START_RUN_HOUR ? parseInt(process.env.START_RUN_HOUR) : new Date().getHours();
            await (new Monitor(lottery, runHour)).init();
        });
        event_bus.on('Turn_off_the_Monitor', async (msg) => {
            ...
            // 如果设置了pinglun环境变量，不创建随机动态，延迟3分钟后切换到下一个账号
            if (utils.hasEnv('pinglun')) {
                log.info('结束抽奖', 'pinglun模式，延迟3分钟后切换到下一个账号');
                await utils.delay(180000); // 延迟3分钟（180000毫秒）
            } else {
