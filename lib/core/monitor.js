...
class Monitor extends Searcher {
     ...
     */
    constructor(lottery_param, runHour) {
        super();
        this.lottery_param = lottery_param;
        this.runHour = runHour; // 保存运行时的小时
    ...
    async init() {
        ...
        if (hasEnv('pinglun')) {
            // 在关闭前推送剩余的跳过URL
            if (process.env.NUMBER == 1 && skippedUrls.length > 0) {
                await sendQmsgOnly(
                    `跳过答题动态`,
                    `帐号备注: root\n\n${skippedUrls.join('\n\n')}`
                );
    ...
    async filterLotteryInfo() {
        ...
        if (hasEnv('pinglun') && process.env.pinglun) {
            ...
            // 否则，使用默认的搜索方式获取动态信息
            protoLotteryInfo = await LotteryInfoMap.get(lottery_param[0])(lottery_param[1], this.runHour);
        }
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            /**屏蔽词 - pinglun模式跳过屏蔽词检查 */
            if (isBlock && !hasPingLunConfig) {
            ...
            if (isAnswerLottery && skip_answer_lottery && !hasPingLunConfig) {
                ...
                if (process.env.NUMBER == 1) {
                    // 收集URL，等待批量推送
                    skippedUrls.push(skipUrl);
                    
                    // 检查是否需要立即推送（距离上次推送超过5分钟或收集了5个URL）
                    const now = Date.now();
                    if (now - lastPushTime > 5 * 60 * 1000 || skippedUrls.length >= 5) {
                        await sendQmsgOnly(
                            `跳过答题动态`,
                            `帐号备注: root\n\n${skippedUrls.join('\n\n')}`
                        );
                    ...
                    const dateStr = now_date.toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' });
