...
class Searcher {
     ...
     * @param {number} runHour 运行时的小时，用于判断应该使用哪种逻辑
     * @returns {Promise<LotteryInfo[] | null>}
     */
    async getLotteryInfoByArticle(key_words, runHour) {
        ...
        for (const { id, pub_time } of cvs) {
            ...
            // 将pub_time转换为北京时间（UTC+8）
            const pub_time_beijing = pub_time + 8 * 3600;
            
            // 使用日期比较，忽略今天，实际天数 = article_create_time
            const now_date = new Date(now_time * 1000);
            const pub_date = new Date(pub_time_beijing * 1000);
            
            // 计算日期差（天数）- 使用本地时间
            const now_date_only = new Date(now_date.getFullYear(), now_date.getMonth(), now_date.getDate());
            const pub_date_only = new Date(pub_date.getFullYear(), pub_date.getMonth(), pub_date.getDate());
            const days_diff = Math.floor((now_date_only - pub_date_only) / (1000 * 60 * 60 * 24));
            
            log.info('获取动态', `专栏${id}时间计算: now=${now_date.getFullYear()}-${now_date.getMonth()+1}-${now_date.getDate()}, pub=${pub_date.getFullYear()}-${pub_date.getMonth()+1}-${pub_date.getDate()}, days_diff=${days_diff}, hour=${runHour}`);
            
            // 12点前：忽略今天；12点后：忽略今天和昨天
            const min_days_diff = runHour < 12 ? 1 : 2;
            
            if (days_diff > article_create_time || days_diff < min_days_diff) {
                if (days_diff < min_days_diff) {
                    const ignore_reason = runHour < 12 ? '今天' : '今天或昨天';
                    log.warn('获取动态', `该专栏(${id})是${ignore_reason}创建的，已忽略`);
                } else {
            ...
            if (content.length < 100) {
                log.warn('获取动态', `专栏(${id})内容获取失败或过短，可能需要更新cookie`);
            }

            // 解析pinglun环境变量，生成筛选映射
        const parsePingLunEnvForFilter = () => {
            const envMap = {};
            if (utils.hasEnv('pinglun')) {
            ...
            for (const dyid of dyid_set) {
                // 如果pinglun环境变量存在且当前动态不在指定列表中，则跳过获取详情
                if (hasPingLunConfig) {
                    ...
                    if (!pinglunFilterMap[dyid] && 
                        ...
                        log.info('获取动态', `非pinglun指定动态，跳过获取详情(https://t.bilibili.com/${dyid})`);
                        continue;
                    }
                }
        ...
        if (dyinfos.length === 0) {
            log.warn('获取动态', `未获取到任何专栏信息，标记账号需要重新运行`);
            process.env.ARTICLE_FAILED = 'true';
        }
