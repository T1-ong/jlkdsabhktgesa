...
class Searcher {
    ...
    async getLotteryInfoByArticle(key_words) {
        ...
        for (const { id, pub_time } of cvs) {
            ...
            const current_hour = new Date(now_time * 1000).getHours();
            ...
            log.info('获取动态', `专栏${id}时间计算: now=${now_date.getFullYear()}-${now_date.getMonth()+1}-${now_date.getDate()}, pub=${pub_date.getFullYear()}-${pub_date.getMonth()+1}-${pub_date.getDate()}, days_diff=${days_diff}, hour=${current_hour}`);
            
            // 12点前：忽略今天；12点后：忽略今天和昨天
            const min_days_diff = current_hour < 12 ? 1 : 2;
            
            if (days_diff > article_create_time || days_diff < min_days_diff) {
                if (days_diff < min_days_diff) {
                    const ignore_reason = current_hour < 12 ? '今天' : '今天或昨天';
