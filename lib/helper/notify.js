...
if (process.env.BARK_PUSH !== undefined) {
...
if (process.env.QMSG_KEY !== undefined) {
    QMSG_KEY = process.env.QMSG_KEY;
}

if (process.env.QMSG_QQ !== undefined) {
...
async function sendNotify(text, desp, params = {}) {
    ...
    log.info('发送通知', `准备发送通知，BARK_PUSH=${BARK_PUSH || '未配置'}, QMSG_KEY=${QMSG_KEY || '未配置'}`);
    //提供多种通知方式
    await Promise.all([
        //微信server酱
            ...
            send({
                ...
                failure: err => {
                    if (retryCount < maxRetries) {
                        retryCount++;
                        log.warn('发送通知', `Bark APP发送通知调用API失败，${retryCount}秒后重试(${retryCount}/${maxRetries})：${err}`);
                        setTimeout(() => sendWithRetry(retryCount), retryInterval * retryCount);
                    } else {
                        log.error('发送通知', `Bark APP发送通知调用API失败！！已重试${maxRetries}次，放弃重试：${err}`);
                        resolve();
                    }
                }
            });
...
async function qmsg(text, desp) {
    const maxRetries = 3; // 最大重试次数
    const retryInterval = 5000; // 重试间隔（毫秒）
    
    return new Promise(resolve => {
        if (!QMSG_KEY) {
            log.debug('发送通知', '您未提供qmsg推送所需的QMSG_KEY，取消qmsg好友推送消息通知');
            resolve();
            return;
        }
        
        // 从NOTE中提取QQ号信息（如果有）
        let targetQQ = QMSG_QQ; // 默认使用配置的QQ号
        if (process.env.NOTE) {
            const qqMatch = process.env.NOTE.match(/qq=(\d+)/);
            if (qqMatch && qqMatch[1]) {
                targetQQ = qqMatch[1]; // 使用note中的QQ号作为目标好友
            }
        }
        
        // 处理desp，移除其中的qq信息（如果有）
        let cleanDesp = desp;
        if (cleanDesp) {
            cleanDesp = cleanDesp.replace(/，qq=\d+/g, '').replace(/,qq=\d+/g, '');
        }
        ...
        const sendWithRetry = (retryCount = 0) => {
            send({
                method: 'POST',
                url: 'http://81.69.229.199:2100/send/' + QMSG_KEY,
                contents: {
                    msg: message,
                    qq: targetQQ
                },
                ...
                success: res => {
                    try {
                        ...
                        if (data.code === 200) {
                            ...
                            // 检查是否是成功的响应（qmsg API返回"操作成功"表示成功）
                            if (data.reason && data.reason.includes('操作成功')) {
                                log.info('发送通知', 'qmsg发送通知消息完成。');
                                resolve();
                                return;
                            }
                            ...
                            if (retryCount < maxRetries) {
                                retryCount++;
                                log.warn('发送通知', `qmsg通知消息失败: ${data.reason}，${retryCount}秒后重试(${retryCount}/${maxRetries})`);
                                setTimeout(() => sendWithRetry(retryCount), retryInterval * retryCount);
                                return;
                            }
                            log.error('发送通知', `qmsg通知消息失败: ${data.reason}，已重试${maxRetries}次`);
                        }
                    } catch (e) {
                        if (retryCount < maxRetries) {
                            retryCount++;
                            log.warn('发送通知', `qmsg通知处理异常: ${e.message}，${retryCount}秒后重试(${retryCount}/${maxRetries})`);
                            setTimeout(() => sendWithRetry(retryCount), retryInterval * retryCount);
                            return;
                        }
                        log.error('发送通知', e);
                ...
                failure: err => {
                    if (retryCount < maxRetries) {
                        retryCount++;
                        log.warn('发送通知', `qmsg通知消息失败！！${err}，${retryCount}秒后重试(${retryCount}/${maxRetries})`);
                        setTimeout(() => sendWithRetry(retryCount), retryInterval * retryCount);
                    } else {
                        log.error('发送通知', `qmsg通知消息失败！！${err}，已重试${maxRetries}次，放弃重试`);
                        resolve();
                    }
                }
            });
...
async function sendQmsgOnly(text, desp, params = {}) {
    if (process.env.NOTE) {
        desp = `帐号备注: ${process.env.NOTE}\n${desp}`;
    }
    ...
    // 只发送Qmsg通知
    await qmsg(text, desp);
}

module.exports = { sendNotify, sendQmsgOnly };
